        ...
        try:
            ...
            for build_round in range(MAX_BUILD_RETRIES + 1):
                ...
                for attempt in range(MAX_RETRIES + 1):
                    ...
                    consolidator = FileConsolidatorAgent(build_dir)
                    consolidate_input = FileConsolidatorInput(build_id=build.id, build_dir=str(build_dir))
                    ...
                    if await self._check_cancelled(build_id): return
                    # ── Phase 4: Security Check (HIGH severity blocking) ───────
                    ...
                    if high_severity_findings:
                        await self._fail(build_id, 
                            f"Build blocked by {len(high_severity_findings)} HIGH severity security findings:\n" + 
