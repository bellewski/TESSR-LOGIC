...
async def chat(payload: PromptChatRequest, db: Session = Depends(get_db)):
    """Send a message to the requirement-refining chatbot. Persists history if template_id provided."""
...
def get_chat_history(template_id: str, db: Session = Depends(get_db)):
    """Retrieve persisted conversation history for a template."""
    svc = PromptService(db)
    history = svc.get_chat_history(template_id)
    return {"template_id": template_id, "messages": history}
