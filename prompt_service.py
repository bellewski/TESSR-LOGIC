...
class PromptService:
    ...
    async def chat(self, messages: list[dict], current_fields: dict | None = None,
                   template_id: str | None = None) -> dict:
        """Chat with the requirement-refining assistant. Persists history to template if template_id given."""
        import re

        # Persist incoming messages to template for resumption after timeouts
        if template_id:
            self._persist_chat_history(template_id, messages)
        ...
        try:
            ...
            if not response.success:
                raise Exception(response.error or "Ollama response failed")
        except Exception as e:
        ...
        # Persist assistant reply to template history
        if template_id:
            full_history = messages + [{"role": "assistant", "content": reply}]
            self._persist_chat_history(template_id, full_history)
    ...
    def _persist_chat_history(self, template_id: str, messages: list[dict]) -> None:
        """Save conversation history to the template so it survives timeouts."""
        tpl = self.get_template(template_id)
        if tpl:
            tpl.conversation_history = json.dumps(messages)
            tpl.updated_at = datetime.now(timezone.utc)
            self.db.commit()

    def get_chat_history(self, template_id: str) -> list[dict]:
        """Retrieve persisted conversation history for a template."""
        tpl = self.get_template(template_id)
        if not tpl or not tpl.conversation_history:
            return []
        try:
            return json.loads(tpl.conversation_history)
        except Exception:
            return []
